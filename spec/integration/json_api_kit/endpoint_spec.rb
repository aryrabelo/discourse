...
RSpec.describe "a JSON:API endpoint", type: :request do
  ...
  describe "the resource a controller serves" do
    it "registers the resource its name matches" do
  ...
  describe "fetching a collection" do
    ...
    it "sends the first page of the collection" do
      expect(parsed_body).to eq(
        {
          data: [
            topic_object(newest, fields:, cursor: cursor_of_record(newest)),
            topic_object(middle, fields:, cursor: cursor_of_record(middle)),
          ],
          included: [],
    ...
    context "when the request sorts by title" do
      ...
      it "sends the rows in that order" do
        expect(parsed_body).to eq(
          {
            data: [
              topic_object(oldest, fields:, cursor: cursor_of_record(oldest, sort:)),
              topic_object(newest, fields:, cursor: cursor_of_record(newest, sort:)),
              topic_object(middle, fields:, cursor: cursor_of_record(middle, sort:)),
            ],
            included: [],
            links: links_of,
          }.deep_stringify_keys,
